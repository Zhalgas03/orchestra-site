  ...
  return (
    <section className="border-b border-white/10 py-24 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-8">
        ...
        {/* Карусель — overflow скрыт только здесь */}
          ...
          >
            {achievements.map((a, idx) => (
              <div
                ...
                className={`flex-shrink-0 border p-8 transition-all duration-500 cursor-default group relative overflow-hidden
                  ${idx === cur
                    ? "border-[#b8860b]/60 bg-[#b8860b]/5"
                    : "border-white/[0.08] bg-white/[0.02] hover:border-white/20"}`}
              >
                <div className={`absolute top-0 right-0 w-12 h-px transition-all duration-300
                  ${idx === cur ? "bg-[#b8860b]" : "bg-transparent group-hover:bg-white/20"}`} />
                <div className={`absolute top-0 right-0 w-px h-12 transition-all duration-300
                  ${idx === cur ? "bg-[#b8860b]" : "bg-transparent group-hover:bg-white/20"}`} />

                <div className={`font-serif text-7xl font-light mb-6 leading-none transition-colors duration-300
                  ${idx === cur ? "text-[#b8860b]" : "text-white/15 group-hover:text-white/25"}`}>
                  {a.num}
                </div>
