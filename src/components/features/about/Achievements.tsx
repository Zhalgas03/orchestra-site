...
const Achievements = () => {
  ...
  useEffect(() => {
    if (trackRef.current) {
      trackRef.current.style.transform = `translateX(-${cur * (cardWidth + 16)}px)`
    }
  }, [cur, cardWidth])

  const goTo = (i: number) => setCur(Math.max(0, Math.min(achievements.length - 1, i)))
        ...
        <div ref={headerRef} className="reveal flex items-end justify-between mb-16">
          ...
          <div className="flex gap-2">
            <button onClick={() => goTo(cur - 1)} disabled={cur === 0}
              ...
              <span className={`absolute inset-0 border transition-colors duration-300 ${cur === 0 ? "border-white/10" : "border-white/20 group-hover:border-[#b8860b]"}`} />
              ...
              <svg width="18" height="12" viewBox="0 0 18 12" fill="none"
                className={`transition-all duration-300 group-hover:-translate-x-0.5 ${cur === 0 ? "text-white/20" : "text-white/50 group-hover:text-[#b8860b]"}`}>
            ...
            <button onClick={() => goTo(cur + 1)} disabled={cur >= achievements.length - 1}
              className="group relative w-14 h-14 flex items-center justify-center transition-all duration-300 disabled:opacity-25 disabled:cursor-not-allowed">
              <span className={`absolute inset-0 border transition-colors duration-300 ${cur >= achievements.length - 1 ? "border-white/10" : "border-white/20 group-hover:border-[#b8860b]"}`} />
              ...
              <svg width="18" height="12" viewBox="0 0 18 12" fill="none"
                className={`transition-all duration-300 group-hover:translate-x-0.5 ${cur >= achievements.length - 1 ? "text-white/20" : "text-white/50 group-hover:text-[#b8860b]"}`}>
          ...
          <div ref={containerRef} className="overflow-hidden">
            <div ref={trackRef} className="flex gap-4"
              ...
              {achievements.map((a, idx) => (
                <div key={a.num} onClick={() => goTo(idx)}
                  style={{ minWidth: `${cardWidth}px`, maxWidth: `${cardWidth}px`, transition: "transform 0.55s cubic-bezier(0.25, 0.46, 0.45, 0.94), opacity 0.55s ease, filter 0.55s ease", ...getCardStyle(idx) }}
                  className={`flex-shrink-0 border p-8 cursor-pointer group relative overflow-hidden ${idx === cur ? "border-[#b8860b]/60 bg-[#b8860b]/5" : "border-white/[0.08] bg-white/[0.02]"}`}>
                  <div className={`absolute top-0 right-0 w-12 h-px transition-colors duration-300 ${idx === cur ? "bg-[#b8860b]" : "bg-transparent"}`} />
                  <div className={`absolute top-0 right-0 w-px h-12 transition-colors duration-300 ${idx === cur ? "bg-[#b8860b]" : "bg-transparent"}`} />
                  <div className={`font-serif text-7xl font-light mb-6 leading-none transition-colors duration-300 ${idx === cur ? "text-[#b8860b]" : "text-white/15"}`}>
                    {a.num}
                  </div>
                  <h3 className="text-white text-lg font-medium mb-3 leading-snug">
                    {t(`about.achievements.items.${a.num}.title`)}
                  </h3>
                  <p className="text-white/50 text-sm leading-relaxed">
                    {t(`about.achievements.items.${a.num}.text`)}
                  </p>
                </div>
              ))}
            </div>
        ...
        <div ref={dotsRef} className="reveal flex items-center gap-3 mt-8">
          {achievements.map((_, i) => (
            <button key={i} onClick={() => goTo(i)}
              className={`transition-all duration-300 rounded-full ${i === cur ? "w-8 h-1 bg-[#b8860b]" : "w-1 h-1 bg-white/25 hover:bg-white/50"}`} />
          ))}
